...
<Aside type="caution" title="Watch out for">
  ...
  action (creating a bid, updating a contact) before confirming, and treat `bids:send`,
  `pay_apps:approve` and `change_orders:approve` on the consent screen as the reason to deny
  if you don't want the model taking those actions.
