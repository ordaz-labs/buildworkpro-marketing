...
- 2026-08-27: Opened a third motion — **targeted outbound to named subcontractors**, documented in `OUTBOUND-PLAN.md`. Rationale: cold interest-targeted Meta prospecting cannot see an audience this narrow, so we build the named list ourselves and let Meta retarget it instead. Glazing is vertical #1. Shipped `/lp/glazing/` and `/solutions/glazing-contractors/` (PR #168, merged) plus the lead engine in `scripts/leadgen/`. SEO/content tracks in this doc continue unchanged.
